...
def server():
    ...
    encrypted_text = aesImplementation(plain_text,key)
    rsa_algo = rsaImplementation(key)
    #print(rsa_algo[0])
    #print(rsa_algo[1][0])
    #print(rsa_algo[1][1])

    send_msg = encrypted_text + "#" + str(rsa_algo[1][0]) + "#" + str(rsa_algo[1][1])
    encrypted_key = pickle.dumps(rsa_algo[0])
    private_key = str(rsa_algo[2][0]) + "#" + str(rsa_algo[2][1])
    ...
    try:
        with open('D:/Don’t Open this/private_key.txt', 'w') as f:
            f.write(private_key)
    ...
    lines = ""
    try:
        with open('D:/Don’t Open this/plain_text.txt') as f:
            lines = f.readline()
    except FileNotFoundError:
        print("Could not create the file.")
    f.close()
...
def printingKeyandtxt(plain_text, key):
...
def aesImplementation(pt,k):
    ...
    if len(plain_text) < block_size :
        ...
        #common_algo
        printingKeyandtxt(plain_text,key)
        key_in_hex = key.encode('utf-8')
        # print(key_in_hex)
        hex_key = key_in_hex.hex()
        plain_text_in_hex = plain_text.encode('utf-8')
        ...
        print("Key : ")
        print(key, "    [in ascii]")
        print(printhex(convertTomatrix(hex_key, 1)), "    [in hex]")
        print()
...
def rsaImplementation(pt):
    ...
    print()
    #print(ciphertext)
    #print(rsaDecryption(ciphertext,d,n))
    r_value = (ciphertext,public_key,private_key)
    return r_value
