...
def receiver():
    ...
    encrypted_key = pickle.loads(s.recv(4096))
    #print(r_msg)
    #print(encrypted_key)

    strings = r_msg.split('#')
    encrypted_text = strings[0]
    print()
    ...
    lines = ""
    try:
        with open('D:/Don’t Open this/private_key.txt') as f:
            lines = f.readline()
    except FileNotFoundError:
        print("Could not create the file.")
    f.close()
    strings2 = lines.split('#')
    d = strings2[0]
    n = strings2[1]
    private_key = (d, n)
    #print(private_key)
    key = rsaDecryption(encrypted_key, int(d), int(n))
    print("Decrypted key is : ")
    print(key)
    print()
    ...
    plain_text = aesdecryption(encrypted_text, key_mat)
    #print(plain_text)

    try:
        with open('D:/Don’t Open this/plain_text.txt', 'w') as f:
            f.write(plain_text)
    except FileNotFoundError:
        print("Could not create the file.")
    f.close()
    s.send("All done".encode())
    s.close()
